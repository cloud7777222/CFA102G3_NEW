...
//				successView.forward(req, res);
		...
		if ("update".equals(action)) {
			...
			try {
				...
				/* 文章編號 */
				Integer postNo = postMessageSvc.getOnePostMessage(mesNo).getPostNo(); // 不能改變, 所以沿用舊資料
				/* 會員編號 */
				...
				//[注意] 此為com.post.model來的修改更新請求(擇一查多)
				PostService postSvc = new PostService();
				if(requestURL.equals("/back_end/post/listMessagesBy_PostNo.jsp"))
					req.setAttribute("listMessagesBy_PostNo", postSvc.getMessagesByPostNo(postNo)); // 資料庫取出的list物件,存入request
				//此為com.postmessage.model來的修改更新請求
				req.setAttribute("postMessageVO", postMessageVO); // 資料庫update成功後,正確的的postMessageVO物件,存入req
				String url = requestURL + "?whichPage=" + whichPage + "&mesNo=" + mesNo; // 送出修改的來源網頁的第幾頁(只用於:listAllPostMessage.jsp.jsp)和修改的是哪一筆
				System.out.println("url=" + url);
				RequestDispatcher successView = req.getRequestDispatcher(url); // 新增成功後轉交至listOnePostMessage.jsp.jsp 顯示
				successView.forward(req, res);
